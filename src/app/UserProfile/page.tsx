...
const calculateBMI = () => {
  ...
  const bmiValue = (weightNum / (heightM * heightM)).toFixed(1); // string
  const bmiValueNum = Number(bmiValue); // number for comparison

  let status = "";
  if (bmiValueNum < 18.5) status = "Underweight 😐";
  else if (bmiValueNum < 25) status = "Normal ✅";
  else if (bmiValueNum < 30) status = "Overweight ⚠️";
  else status = "Obese 🚨";
